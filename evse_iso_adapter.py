...
class EVSE_ISO_Adapter(EVSEControllerInterface): #EVSEControllerInterface from Ecog-io
    ...
    async def get_service_parameter_list(self, service_id: int) -> Optional[ServiceParameterList]:
        ...
        try:
            ...
            nominal_voltage_parameter = Parameter(name="EVSENominalVoltage", int_value=self.controller.nominalMaxVoltage)
    ...
    async def get_cp_state(self) -> CpState:
        ...
        logger.info(f"getting CP STATE {self.controller.basicCharging.committed_state}")
        return self.controller.basicCharging.committed_state
    ...
    async def get_ac_charge_params_v2(self) -> ACEVSEChargeParameter:
        ...
        evse_nominal_voltage = PVEVSENominalVoltage(
            multiplier=0, value=self.controller.nominalMaxVoltage, unit=UnitSymbol.VOLTAGE
        )
        evse_max_current = PVEVSEMaxCurrent(
            multiplier=0, value=self.controller.nominalMaxCurrent, unit=UnitSymbol.AMPERE
        )
    ...
    async def get_ac_charge_params_v20(
        ...
        ac_charge_parameter_discovery_res_params = ACChargeParameterDiscoveryResParams(
            evse_max_charge_power=RationalNumber.get_rational_repr(self.controller.nominalMaxPower),
        ...
        if energy_service == ServiceV20.AC:
            ...
            return BPTACChargeParameterDiscoveryResParams(
                **(ac_charge_parameter_discovery_res_params.dict()),
                evse_max_discharge_power=RationalNumber.get_rational_repr(self.controller.nominalMaxPower),
    ...
    async def get_evse_max_power_limit(self) -> PVEVSEMaxPowerLimit:
        return PVEVSEMaxPowerLimit(multiplier=1, value=self.controller.nominalMaxPower, unit="W")
